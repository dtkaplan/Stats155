...
```{r}
trials = do(100)*mm(wage~sex, data=resample(CPS85))
...
```{r}
mod = mm( time ~ sex, data=sample(TenMileRace, size=800))
confint(mod)
```
...
Example:
```{r cache=TRUE}
trials = do(200)*mm( net ~ sex, 
                     data=sample(TenMileRace,size=800))
sd(~F, data=trials)
sd(~M, data=trials)
```
...
sd( ~M, data=trials) # Standard error
...
```{r}
mod3 = mm( wage ~ sex + sector + cut(age, breaks=c(0,20,40,60,80)), data=CPS85)
```
...
```{r}
tally( ~ sex + sector + ntiles(age), data=CPS85)
```
...
Do single people earn less than married people according to the 1985 CPS survey?
```{r}
mod = mm(wage ~ married, data=CPS85)
coef(mod)
```
...
```{r}
mod2 = mm(age ~ sex, data=CPS85)
confint(mod2)
```

What might the other factors be at work here?
* Age
