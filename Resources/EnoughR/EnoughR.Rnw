...
For other things \verb+y ~ x | z+ can usually be interpreted to mean ``\verb+y+ depends on \verb+x+ separately for each \verb+z+.''
...
Simple distribution
<<densityplot1,fig.height=2.4,fig.keep="none">>=
densityplot( ~ age, data=CPS85 )
@
Overlaying two (or more) groups
<<densityplot2,fig.height=2.4,tidy=FALSE>>=
densityplot( ~ age, groups=sex, 
             data=CPS85, auto.key=TRUE)
@
\vspace*{-.2in}
Side-by-side plots:
<<densityplot3,fig.height=2.4,fig.keep="none">>=
